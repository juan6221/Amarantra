  ...
  return (
    <div className="card-dark">
      <div className="flex items-start justify-between mb-4">
        <div className="w-10 h-10 rounded-lg bg-gold-400/10 flex items-center justify-center text-gold-400">{icon}</div>
        {trend !== undefined && (
          <span className={`text-xs font-semibold px-2 py-1 rounded-full ${trend >= 0 ? 'text-emerald-400 bg-emerald-400/10' : 'text-red-400 bg-red-400/10'}`}>
            {trend >= 0 ? '+' : ''}{trend}%
          </span>
        )}
      </div>
    ...
    <div className="bg-dark-700 border border-dark-500 rounded-lg px-4 py-3 shadow-xl text-sm">
      ...
      {payload.map((p, i) => (
        <p key={i} style={{ color: p.color }} className="font-semibold">
          {p.name}: {p.value > 1000 ? `$${p.value.toLocaleString('es-CO')}` : p.value}
        </p>
      ))}
    </div>
  )
...
export default function DashboardPage() {
  ...
  const { productos, loading: lp } = useProductos()
  const { categorias, loading: lc } = useCategorias()
  const { usuarios, loading: lu }   = useUsuarios()
  const { ventas, loading: lv }     = useVentas(user?.id, 'admin')

  const loading = lp || lc || lu || lv
  ...
  const topProductos = useMemo(() => {
    const map = {}
    ventas.forEach(v => v.productos.forEach(({ productoId, cantidad }) => {
      map[productoId] = (map[productoId] || 0) + cantidad
    }))
    return Object.entries(map)
      .map(([id, cantidad]) => ({ nombre: productos.find(p => p.id === +id)?.nombre || '?', cantidad }))
  ...
  const ventasMensuales = useMemo(() => {
    const map = {}
    ventas.forEach(v => {
      const mes = new Date(v.fecha).toLocaleString('es-CO', { month: 'short' })
      if (!map[mes]) map[mes] = { mes, ventas: 0, transacciones: 0 }
      map[mes].ventas       += v.total
      map[mes].transacciones += 1
    })
    return Object.values(map)
  }, [ventas])

  if (loading) return <div className="page-container"><LoadingSpinner text="Cargando dashboard..." /></div>
    ...
    <div className="page-container animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
        <StatCard
          ...
          label="Ventas Totales" value={`$${stats.totalVentas.toLocaleString('es-CO')}`} sub={`${ventas.length} transacciones`} trend={12} />
        <StatCard
          ...
          label="Productos Activos" value={stats.productosActivos} sub={`${stats.sinStock} sin stock`} trend={5} />
        <StatCard
          ...
          label="Categorías" value={categorias.filter(c => c.activo).length} sub="categorías activas" />
        <StatCard
          ...
          label="Usuarios Activos" value={stats.usuariosActivos} sub="en la plataforma" trend={0} />
      </div>

      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-6">
        <div className="xl:col-span-2 card-dark">
          <h3 className="font-semibold text-white mb-6">Ventas por Período</h3>
          <ResponsiveContainer width="100%" height={260}>
            <AreaChart data={ventasMensuales} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
              <defs>
              ...
              <Area type="monotone" dataKey="ventas" name="Ventas" stroke="#d4af37" strokeWidth={2.5} fill="url(#goldArea)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="card-dark">
          ...
          <ResponsiveContainer width="100%" height={260}>
            <PieChart>
              <Pie data={ventasPorCategoria} cx="50%" cy="45%" innerRadius={55} outerRadius={85} paddingAngle={3} dataKey="value">
                {ventasPorCategoria.map((_, i) => <Cell key={i} fill={GOLD_COLORS[i % GOLD_COLORS.length]} />)}
              </Pie>
              <Tooltip content={<CustomTooltip />} />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <div className="card-dark">
          ...
          {topProductos.length === 0 ? <p className="text-dark-400 text-sm text-center py-8">Sin ventas registradas.</p> : (
        ...
        <div className="card-dark">
          <h3 className="font-semibold text-white mb-6">Transacciones por Período</h3>
          {ventasMensuales.length === 0 ? <p className="text-dark-400 text-sm text-center py-8">Sin ventas registradas.</p> : (
            <ResponsiveContainer width="100%" height={220}>
