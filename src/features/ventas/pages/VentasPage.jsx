...
import { useAppStore, useAuthStore } from '../../../shared/store/useStore'
    ...
    <div className="flex items-center gap-3 py-3 border-b border-dark-600 last:border-0">
      ...
      <div className="flex items-center gap-2">
        <button onClick={() => onUpdate(Math.max(1, item.cantidad - 1))}
          className="w-7 h-7 rounded-full bg-dark-600 hover:bg-dark-500 text-white flex items-center justify-center text-sm transition-colors">
          −
        </button>
...
export default function VentasPage() {
  const { productos, categorias, ventas, addVenta } = useAppStore()
  ...
  const procesarVenta = () => {
    ...
    const venta = { usuarioId: user.id, productos: carrito, total }
    addVenta(venta)
    setLastVenta({ ...venta, id: Date.now(), fecha: new Date().toISOString().split('T')[0] })
    setCarrito([])
    setModalSuccess(true)
  }

  const miVentas = ventas.filter(v => user?.rol === 'admin' || v.usuarioId === user?.id)
    .sort((a, b) => b.id - a.id)
    ...
    <div className="page-container animate-fade-in">
        ...
        {[
          ...
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`px-5 py-2 rounded-lg text-sm font-medium transition-all ${
              tab === t.id ? 'bg-gold-gradient text-dark-900' : 'text-dark-300 hover:text-white'
            }`}>
            {t.label}
          </button>
        ))}
          ...
          <div className="xl:col-span-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {productosDisponibles.map(p => {
                ...
                return (
                  <div key={p.id}
                    className={`flex gap-3 bg-dark-800 border rounded-xl p-3 cursor-pointer transition-all
                      ${inCart ? 'border-gold-400/50 bg-gold-400/5' : 'border-dark-600 hover:border-gold-400/30'}`}
                    onClick={() => addToCart(p)}>
                    <div className="w-16 h-16 rounded-lg overflow-hidden bg-dark-700 flex-shrink-0">
                      {p.imagenes?.[0] && (
                        <img src={p.imagenes[0]} alt="" className="w-full h-full object-cover" onError={e => { e.target.style.display = 'none' }} />
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                    ...
                    {inCart && (
                      <div className="flex items-start">
                        <span className="w-5 h-5 bg-gold-gradient text-dark-900 rounded-full text-xs font-bold flex items-center justify-center">
                          {inCart.cantidad}
                        </span>
                      </div>
                    )}
            ...
            <div className="bg-dark-800 border border-dark-600 rounded-xl p-5 sticky top-6">
              ...
              {carrito.length === 0 ? (
                ...
                <>
                  <div className="mb-4 max-h-80 overflow-y-auto scrollbar-thin pr-1">
                    {carrito.map(item => (
                      <CarritoItem key={item.productoId} item={item}
                        ...
                        onUpdate={v => updateItem(item.productoId, v)}
                        onRemove={() => removeItem(item.productoId)} />
                    ))}
                  </div>
                  ...
                  <button onClick={procesarVenta} className="btn-gold w-full">
                    Procesar venta
                  </button>
      ...
      {tab === 'historial' && (
        <div className="space-y-3">
          {miVentas.length === 0 && (
            <div className="text-center py-16 text-dark-400">No hay ventas registradas.</div>
          )}
          {miVentas.map(v => (
      ...
      <Modal isOpen={modalSuccess} onClose={() => setModalSuccess(false)} title="¡Venta procesada!" size="sm">
        <div className="text-center py-4">
          ...
          {lastVenta && (
            <p className="text-gold-400 font-bold text-2xl">${lastVenta.total?.toLocaleString('es-CO')}</p>
          )}
          <p className="text-dark-400 text-sm mt-2">{lastVenta?.productos?.length} producto(s) procesado(s)</p>
          <button onClick={() => setModalSuccess(false)} className="btn-gold mt-6 w-full">
            Continuar
          </button>
        </div>
      </Modal>
